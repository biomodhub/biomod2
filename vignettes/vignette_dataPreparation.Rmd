...
Species in example 2) is widely dispersed within Europe. Extent 2a) will not be sufficient to model the niche of the species as some environmental conditions might be missing (species present elsewhere) or not enough different from the ones in which the species is present. Extent 3b) would be more appropriate to also take into account environmental conditions not favorable for the species.
...
Species distribution models build statistical relationships between presence and absence points and their corresponding environmental values. As any statistical method, it requires a minimum number of points to work and provide (enough) reliable results. Common arbitrary rule is to have at the very least 5 occurrences for each variable added in the model (e.g. if 6 variables are used, 6 * 5 = 30 occurrences should be available).
