...
#### 4.2-5 Tuning and Model Options

##### 4.2-4-10 Bugfix
- fixed `wrap` applied to a `data.frame` in `BIOMOD_Projection`

##### 4.2-4-9 Bugfix
- fixed `predict` method for `RF` with `do.classif = FALSE`

##### 4.2-4-8 Bugfix
- improved tests in `bm_PlotEvalMean`

##### 4.2-4-7 Bugfix
- fixed `do.classif` ignored in `BIOMOD_ModelingOptions`

##### 4.2-4-6 Bugfix
- fixed parallelization in `BIOMOD_Projection`

##### 4.2-4-5 Bugfix
- fixed summary method for `BIOMOD.formated.data` and `BIOMOD.formated.data.PA`

##### 4.2-4-4 Bugfix
- fixed `bm_PlotResponseCurves` for ensemble models merged by algo (for Maxent)
...
##### 4.2-4-2 Bugfix
- set xgboost `verbose = 0` (from `verbose = 1`)

##### 4.2-4-1 Bugfix
- fixed `BIOMOD_FormatingData` checks for `resp.xy`
