                     ...
                     W = NULL)
{
  ...
  Eval = data.frame(matrix(
    0,
    ncol = 3,
    nrow = 16,
    dimnames = list(NULL, c("Size", "Decay", "AUC"))
  ))
  Eval[, 1] = rep(size, 4)
  ...
  for (i in 1:nbCV) {
    ...
    if (is.null(W)) {
      Eval[, 3] = Eval[, 3] + apply(
        ...
        Target,
        Input,
        FUN = function(x, Samp, Target, Input) {
          nn = nnet(
            eval(parse(
              text = paste(
                "Target[Samp$calibration]",
                paste(.scopeExpSyst(Input[1:10, , drop =
                                            FALSE], "GBM"), collapse = "")
              )
            )),
          ...
          AUC = as.numeric(pROC::auc(pROC::roc(Target[Samp$evaluation], predict(nn, Input[Samp$evaluation, , drop =
                                                                                            FALSE]))))
          return(AUC)
        }
      )
            ...
            AUC <-
              as.numeric(pROC::auc(
                pROC::roc(
                  ...
                  as.numeric(predict(nn, Input[Samp$evaluation, , drop =
                                                 FALSE])),
                  levels = c(0, 1),
                  direction = '<'
                )
