...
'BIOMOD_Projection' <- function(modeling.output,
  ...
  if(build.clamping.mask){
    ...
    if(output.format == '.RData'){
      save(list=nameMask, file = file.path(namePath, paste(nameProj, "_ClampingMask", output.format ,sep="")), compress=compress)
    } else {
  ...
  proj <- lapply(selected.models, function(mod.name){
    ...
    filename <- NULL
    if(!do.stack){
      filename <- file.path(namePath, "individual_projections", paste0(nameProj, "_", mod.name, ifelse(output.format==".RData",".grd",output.format)) )
    }
  ...
  if(output.format == '.RData'){
    saved.files <- file.path(namePath, paste(nameProj,"_", modeling.output@sp.name, output.format ,sep=""))
    save(list = nameProjSp, file = saved.files, compress=compress)
  } else {
    if(do.stack){
      saved.files <- file.path(namePath, paste(nameProj,"_", modeling.output@sp.name, output.format ,sep=""))
      writeRaster(x=get(nameProjSp),
