...
setMethod('show', signature('biomod2_ensemble_model'), function(object) { callNextMethod(object) })
          ...
          function(object, newdata, ...)
          {
            predfun <- function(newdata, on_0_1000, mod.name, na.rm, ...) {
              ...
              if (is.null(mean_prediction)) { 
                mean_prediction <- app(newdata, mean, wopt = list(names = mod.name), na.rm = na.rm)
              }
              if (is.null(sd_prediction)) { 
                sd_prediction <- app(newdata, sd, wopt = list(names = mod.name), na.rm = na.rm) 
              }
              ...
              if (data.type == "binary") {
                if (on_0_1000) {
                  ci_prediction <- classify(round(ci_prediction),
                                            matrix(c(-Inf, 0, 0, 1000, Inf, 1000),
                                                   nrow = 2, byrow = TRUE))
                } else {
                  ci_prediction <- classify(round(ci_prediction),
                                            matrix(c(-Inf, 0, 0, 1, Inf, 1),
                                                   nrow = 2, byrow = TRUE))
                }
              }
              ci_prediction
