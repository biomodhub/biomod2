...
multiple.plot <- function(Data, coor, color.gradient = 'red', plots.per.window = 9, cex = 1, save.file = "no"
                          , name = "multiple plot", ImageSize = "small", AddPresAbs = NULL, PresAbsSymbol = c(cex * 0.8, 16, 4))
{
  ...
  if (show.scale) {
    ...
    if (SRC) {
      ...
      if (level.range[1] == min(data.in)) {
        ...
        lmin <- paste0(round(level.range[1], digits = 2), " or lower")
      }
      if (level.range[2] == max(data.in)) {
        lmax <- round(level.range[2], digits = 2)
      } else {
        lmax <- paste0(round(level.range[2], digits = 2), " or over")
      }
      ...
      legend.txt = list(lmax, '', '', '', '',
                        ...
                        lmin)
      if (!multiple.plot) {
        legend(0.2, 0.92, legend = legend.txt, cex = 1, bty = 'n'
               , fill = rev(color.system[c(1, seq(2, 101, length.out = 19), 102)]))
      } else {
        legend(0.2, 1.05, legend = legend.txt, cex = cex, bty = 'n'
               , fill = rev(color.system[c(1, seq(2, 101, length.out = 19), 102)]))
      }
    }
  } else {
